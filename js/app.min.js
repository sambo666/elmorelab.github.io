...
/* WEBPACK VAR INJECTION */(function(global, module) {/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "iconMenu", function() { return iconMenu; });
...
/* harmony import */ var _babel_runtime_helpers_toConsumableArray__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(1);
/* harmony import */ var _babel_runtime_helpers_toConsumableArray__WEBPACK_IMPORTED_MODULE_1___default = /*#__PURE__*/__webpack_require__.n(_babel_runtime_helpers_toConsumableArray__WEBPACK_IMPORTED_MODULE_1__);
/* harmony import */ var _babel_runtime_helpers_typeof__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(4);
...
window.Element && !Element.prototype.closest && (Element.prototype.closest = function (e) {
  ...
  'function' == typeof define && __webpack_require__(18) ? define([], function () {
    return t(e);
  }) : 'object' == (typeof exports === "undefined" ? "undefined" : _babel_runtime_helpers_typeof__WEBPACK_IMPORTED_MODULE_2___default()(exports)) ? module.exports = t(e) : e.SmoothScroll = t(e);
    ...
    M.cancelScroll = function (e) {
      ...
      if (u || t) {
        ...
        0 === q.pageYOffset && q.scrollTo(0, 0), f = i, h = s, u || history.pushState && h.updateURL && history.pushState({
          ...
          anchor: f.id
        }, document.title, f === document.documentElement ? '#top' : location.pathname + '#' + f.id), 'matchMedia' in q && q.matchMedia('(prefers-reduced-motion)').matches ? q.scrollTo(0, Math.floor(g)) : (H('scrollStart', s, i, c), M.cancelScroll(!0), q.requestAnimationFrame(b));
      }
    };
    var t = function t(e) {
...
// Slider Captcha
var captchas = document.querySelectorAll('.jk-slider-field');
if (captchas) {
  var _loop = function _loop(i) {
    var bot = true;
    var wrapper = captchas[i].querySelector('.jk-slider-wrapper');
    var slider = wrapper.querySelector('.jk-slider');
    ...
    function onmousemove() {
      ...
      var slideLeft;
      isMobile.any() ? slideLeft = event.changedTouches[0] : slideLeft = event;
      var left = slideLeft.pageX - w_offset.left - this.offsetWidth / 2;
      if (left <= 0) left = 0;else if (left >= maxleft) left = maxleft;
      this.style.left = left + 'px';
      if (left == maxleft) {
        wrapper.classList.add('human');
        document.querySelector('.jk-slider-field input').value = 'notbot';
        bot = false;
      } else {
        wrapper.classList.remove('human');
        document.querySelector('.jk-slider-field input').value = 'isbot';
        bot = true;
      }
    }
  };
  ...
  // 	if (document.querySelector('.jk-slider-field input').value != "notbot") {
...
function _goto(target_block, speed) {
  ...
  //var anchor = document.querySelector('#allcomments');
  //console.log(anchor)
  //scr.animateScroll(anchor);
}
...
testWebP(function (support) {
  if (support === true) {
    document.body.classList.add('_webp');
  } else {
    document.body.classList.add('_no-webp');
  }
});
function ibg() {
  if (isIE()) {
...
if (iconMenu != null) {
  ...
  document.addEventListener("click", function (event) {
    ...
    if (event.target.matches(".menu__body") || !event.target.closest(".menu__icon")) {
      closeMenu();
    }
  }, false);
}
...
//=================
//Menu Go To
// from <a data-goto=".page__section-1" href="#" class="menu__link">
// to		<section class="page__section page__section-1">
...
function body_lock_remove(delay) {
  ...
  if (unlock) {
    ...
    setTimeout(function () {
      for (var _index9 = 0; _index9 < lock_padding.length; _index9++) {
        var el = lock_padding[_index9];
...
function body_lock_add(delay) {
  ...
  if (unlock) {
    var lock_padding = document.querySelectorAll('._lp');
    for (var _index10 = 0; _index10 < lock_padding.length; _index10++) {
      var el = lock_padding[_index10];
...
var lazyImages = _babel_runtime_helpers_toConsumableArray__WEBPACK_IMPORTED_MODULE_1___default()(document.querySelectorAll('img[data-src], source[data-srcset]'));
...
function getMap() {
  var loadMapBlockPos = loadMapBlock.getBoundingClientRect().top + pageYOffset;
  if (pageYOffset > loadMapBlockPos - windowHeight) {
    var loadMapUrl = loadMapBlock.dataset.map;
    if (loadMapUrl) {
      loadMapBlock.insertAdjacentHTML('beforeend', "<iframe src=\"".concat(loadMapUrl, "\" style=\"border:0\" allowfullscreen=\"\" loading=\"lazy\"></iframe>"));
...
function _getContent() {
  _getContent = _babel_runtime_helpers_asyncToGenerator__WEBPACK_IMPORTED_MODULE_0___default()( /*#__PURE__*/_babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_3___default.a.mark(function _callee() {
    var response, result;
...
//lastVisibleElement
function lastVisibleElement(selectors) {
  var allVisible = _babel_runtime_helpers_toConsumableArray__WEBPACK_IMPORTED_MODULE_1___default()(document.querySelectorAll(selectors)).filter(function (s) {
    return !_is_hidden(s);
  });
  if (allVisible.length !== 0) {
    return allVisible.shift();
  } else {
    return null;
  }
}
// __goto
function __goto(block) {
  var gotoBlock = document.querySelector(block);
  var gotoBlockValue = gotoBlock.getBoundingClientRect().top + pageYOffset;
  window.scrollTo({
    top: gotoBlockValue - 200,
    behavior: 'smooth'
  });
}
//Forms
var forms = document.querySelectorAll('form');
// const checkInput = document.querySelector('.checkbox__input');
// if (checkInput) {
// 	const formEmail = document.getElementById('formEmail'),
// 				emailParent = formEmail.parentElement;
// 	checkInput.addEventListener('change', (e) => {
// 		if (checkInput.checked) {
// 			emailParent.classList.remove('_hidden');
// 			formEmail.classList.add('_req');
// 		} else {
// 			formEmail.classList.remove('_req', '_error');
// 			emailParent.classList.add('_hidden');
// 		}
// 	});
// }
if (forms.length > 0) {
  for (var _index11 = 0; _index11 < forms.length; _index11++) {
    var el = forms[_index11];
...
function _form_submit() {
  _form_submit = _babel_runtime_helpers_asyncToGenerator__WEBPACK_IMPORTED_MODULE_0___default()( /*#__PURE__*/_babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_3___default.a.mark(function _callee2(e) {
    var btn, form, token, error, formAction, formMethod, message, ajax, test, popupForm, formMsg, formData, response, result, formHeight, form_error;
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          case 0:
            ...
            test = form.getAttribute('data-test');
            popupForm = form.getAttribute('data-popup');
            if (popupForm) {
              formMsg = form.nextSibling.nextSibling;
            }
            //const lastComment = lastVisibleElement('.comment__wrapper', );

            // if (lastComment !== null) {
            // 	insertAfter(messageOk, lastComment);
            // } else {
            // 	const commentsContainer = document.querySelector('.comments-container');
            // 	commentsContainer.insertAdjacentElement('afterbegin', messageOk);
            // }
            // let pForm;
            // if (popupForm) {
            // 	pForm = form.closest('.popup__form')
            // 	const pFormHeight = pForm.offsetHeight;
            // 	pForm.style.height = `${pFormHeight}px`;
            // }
            //insertBefore(messageOk, commentsShow);
            //SendForm
            if (!ajax) {
              _context2.next = 34;
              break;
          ...
          case 23:
            ...
            if (popupForm) {
              form.classList.add('_hide-form');
              formHeight = form.offsetHeight;
              form.style.height = "".concat(formHeight, "px");
              window.setTimeout(function () {
                ...
                window.setTimeout(function () {
                  ...
                  window.setTimeout(function () {
                    popup_close();
                    window.setTimeout(function () {
                      form.classList.remove('_hide-form');
                      formMsg.style.opacity = null;
                      form.style.height = null;
                      formMsg.style.display = null;
                    }, 300);
                  }, 3500);
                }, 200);
              }, 300);
...
function form_validate(form) {
  ...
  if (form_req.length > 0) {
    for (var _index12 = 0; _index12 < form_req.length; _index12++) {
      var _el3 = form_req[_index12];
      if (!_is_hidden(_el3)) {
        error += form_validate_input(_el3);
      }
    }
  }
  return error;
...
function phone_test(input) {
  var replacer = /[ -]/g;
  var val = input.value.replace(replacer, '');
  //return !/^[0-9- ]{9,12}$/.test(input.value);
...
function form_validate_input(input) {
  ...
  } else if (input.getAttribute('name') == 'captcha' || input.classList.contains('_captcha')) {
...
function form_clean(form) {
  var inputs = form.querySelectorAll('input[class~=_clear],textarea');
  for (var _index13 = 0; _index13 < inputs.length; _index13++) {
    var _el4 = inputs[_index13];
    _el4.parentElement.classList.remove('_focus');
    _el4.classList.remove('_focus');
    _el4.value = _el4.getAttribute('data-value');
  }
...
//=================
//Popups
var popup_link = document.querySelectorAll('._popup-link2');
var popups = document.querySelectorAll('.popup2');
var _loop6 = function _loop6(_index18) {
  var el = popup_link[_index18];
  el.addEventListener('click', function (e) {
    e.stopPropagation();
    e.preventDefault();
    if (unlock) {
...
for (var _index18 = 0; _index18 < popup_link.length; _index18++) {
  _loop6(_index18);
}
for (var _index19 = 0; _index19 < popups.length; _index19++) {
  var popup = popups[_index19];
...
function popup_open(item) {
  ...
  if (curent_popup && unlock) {
    ...
    wrapper.classList.add('blur-filter');
...
function popup_close(item) {
  ...
  if (unlock) {
    if (!item) {
      for (var _index20 = 0; _index20 < popups.length; _index20++) {
        var _popup = popups[_index20];
    ...
    var wrapper = document.querySelector('.wrapper');
    wrapper.classList.remove('blur-filter');
...
if (popup_close_icon) {
  var _loop7 = function _loop7(_index21) {
    var el = popup_close_icon[_index21];
    el.addEventListener('click', function () {
      popup_close(el.closest('.popup'));
    });
  };
  for (var _index21 = 0; _index21 < popup_close_icon.length; _index21++) {
    _loop7(_index21);
  }
}
...
/*
	...
	Если нужно включать\выключать работу спойлеров на разных экранах
...
/* WEBPACK VAR INJECTION */}.call(this, __webpack_require__(16), __webpack_require__(17)(module)))
...
// EXTERNAL MODULE: ./app/js/_functions.js
...
// CONCATENATED MODULE: ./app/js/plugins/_translator.js
...
var _translator_Translator = /*#__PURE__*/function () {
  function Translator() {
  ...
  createClass_default()(Translator, [{
    ...
    value: function _fetch(path) {
      return fetch(path).then(function (response) {
        ...
        console.error("Could not load ".concat(path, ". Please make sure that the file exists."));
      });
    }
  }, {
    key: "_getResource",
    value: function () {
      var _getResource2 = asyncToGenerator_default()( /*#__PURE__*/regenerator_default.a.mark(function _callee(lang) {
        var translation;
          ...
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                ...
                return _context.abrupt("return", JSON.parse(this._cache.get(lang)));
              case 2:
                _context.next = 4;
                return this._fetch("".concat(this._options.filesLocation, "/").concat(lang, ".json"));
              case 4:
      ...
      function _getResource(_x) {
        return _getResource2.apply(this, arguments);
      }
      return _getResource;
    }()
  }, {
    key: "load",
    value: function () {
      var _load = asyncToGenerator_default()( /*#__PURE__*/regenerator_default.a.mark(function _callee2(lang) {
          ...
          while (1) {
            switch (_context2.prev = _context2.next) {
              ...
              case 3:
                _context2.t0 = this;
                _context2.next = 6;
                return this._getResource(lang);
              case 6:
                _context2.t1 = _context2.sent;
                _context2.t0._translate.call(_context2.t0, _context2.t1);
    ...
    value: function () {
      var _getTranslationByKey = asyncToGenerator_default()( /*#__PURE__*/regenerator_default.a.mark(function _callee3(lang, key) {
        var translation;
          ...
          while (1) {
            switch (_context3.prev = _context3.next) {
              case 0:
                ...
                throw new Error("Expected a key to translate, got nothing.");
              case 2:
                if (!(typeof key != "string")) {
                  _context3.next = 4;
                  break;
                }
                throw new Error("Expected a string for the key parameter, got ".concat(typeof_default()(key), " instead."));
              case 4:
                _context3.next = 6;
                return this._getResource(lang);
              case 6:
                translation = _context3.sent;
                return _context3.abrupt("return", this._getValueFromJSON(key, translation, true));
              case 8:
              case "end":
      ...
      function getTranslationByKey(_x3, _x4) {
        return _getTranslationByKey.apply(this, arguments);
      }
      return getTranslationByKey;
    }()
  }, {
    key: "_getValueFromJSON",
    value: function _getValueFromJSON(key, json, fallback) {
      var text = key.split(".").reduce(function (obj, i) {
        return obj[i];
      }, json);
      if (!text && this._options.defaultLanguage && fallback) {
        var fallbackTranslation = JSON.parse(this._cache.get(this._options.defaultLanguage));
        text = this._getValueFromJSON(key, fallbackTranslation, false);
      } else if (!text) {
        text = key;
        console.warn("Could not find text for attribute \"".concat(key, "\"."));
      }
      return text;
    }
  }, {
    ...
    value: function _translate(translation) {
      ...
      var nullSafeSplit = function nullSafeSplit(str, separator) {
        return str ? str.split(separator) : null;
      };
      var replace = function replace(element) {
        ...
        if (keys.length > 0 && keys.length !== properties.length) {
          ...
          pairs.forEach(function (pair) {
            var _pair = slicedToArray_default()(pair, 2),
              key = _pair[0],
              property = _pair[1];
            var text = _this._getValueFromJSON(key, translation, true);
            if (text) {
              element[property] = text;
              element.setAttribute(property, text);
            } else {
              console.error("Could not find text for attribute \"".concat(key, "\"."));
            }
          });
        }
      };
...
/* harmony default export */ var _translator = (_translator_Translator);
...
window.onload = function () {
  ...
  var heroBanner = document.querySelector('.hero__banner');
  //console.log(window.getComputedStyle(heroBanner, null).display)
  if (heroBanner && window.getComputedStyle(heroBanner, null).display === 'block') {
    var heroControls = document.querySelector('.hero__controls');
    var heroBannerSizing = heroBanner.getBoundingClientRect();
    //console.log(heroBannerSizing)
    if (heroControls && heroBannerSizing.top > -40) {
      heroControls.style.top = heroBannerSizing.top + heroBannerSizing.height + 'px';
    }
  }
  var breakpoint = window.matchMedia('(min-width:62rem)');

  // Swiper Timeline

  var myTimeSwiper;
  var timeSwiper = document.querySelector('.swiper-timeline');
  if (timeSwiper) {
    var enableTimeSwiper = function enableTimeSwiper() {
      if (myTimeSwiper !== undefined) {
        swiperWrapper.style.display = 'flex';
        timelineContainer.style.display = 'none';
        timelineRow.style.display = 'flex';
      }
      var row2 = timeSwiper.querySelector('.row-2');
      var row1 = timeSwiper.querySelector('.row-1');
      var row1Slides = toConsumableArray_default()(row1.querySelectorAll('.swiper-slide'));
      var arr1Slides = row1Slides.map(function (el) {
        return el.cloneNode(true);
      });
      var row2Slides = toConsumableArray_default()(row2.querySelectorAll('.swiper-slide'));
      var arr2Slides = row2Slides.map(function (el) {
        return el.cloneNode(true);
      });
      arr1Slides.forEach(function (el) {
        return swiperWrapper.insertAdjacentElement('beforeend', el);
      });
      arr2Slides.forEach(function (el) {
        return swiperWrapper.insertAdjacentElement('beforeend', el);
      });
      swiperWrapper.after(timelineRow);
      timelineContainer.style.display = 'none';
      var enableSwiper = function enableSwiper() {
        var _SwiperCore;
        var slidesPerView = [1.5, 'auto', 'auto', 3];
        myTimeSwiper = new core_class('.swiper-timeline', (_SwiperCore = {
          //observer: true,
          //observeParents: true,
          //speed: 500,
          //slidesPerView: 'auto',
        ...
        }, defineProperty_default()(_SwiperCore, "autoplay", false), defineProperty_default()(_SwiperCore, "slideToClickedSlide", true), defineProperty_default()(_SwiperCore, "breakpoints", {
          320: {
            slidesPerView: slidesPerView[0]
          },
          390: {
        ...
        myTimeSwiper.on('slideChange', function () {});
      };
      enableSwiper();
    };
    var swiperWrapper = timeSwiper.querySelector('.swiper-wrapper');
    var timelineContainer = document.querySelector('.timeline__container');
    var timelineRow = document.querySelector('.timeline-row').cloneNode(true);
    timelineRow.classList.add('swiper-line');
    var breakpointChecker = function breakpointChecker() {
      if (breakpoint.matches === true) {
        ...
        if (myTimeSwiper !== undefined) {
          ...
          //myTimeSwiper.destroy( true, true );
        }

        return;
      } else if (breakpoint.matches === false) {
        console.log('enableTimeSwiper');
        enableTimeSwiper();
        //return enableSwiper();
      }
    };
  ...
  if (swiperEl) {
    ...
    var enableSwiper = function enableSwiper() {
      var _SwiperCore2;
      var slidesPerView = [1.4, 2, 2];
      mySwiper = new core_class('.swiper', (_SwiperCore2 = {
        //observer: true,
        //observeParents: true,
        //speed: 500,
        //slidesPerView: 'auto',
      ...
      }, defineProperty_default()(_SwiperCore2, "autoplay", false), defineProperty_default()(_SwiperCore2, "slideToClickedSlide", true), _SwiperCore2));
      mySwiper.on('slideChange', function () {});
    };
    breakpoint.addEventListener("change", function () {
      return _breakpointChecker();
    });
    // kickstart
    _breakpointChecker();
  }

  // locale select script
  setTimeout(function () {
    var translator = new _translator({
      persist: true,
      languages: ["en", "ru"],
      defaultLanguage: "en",
      detectLanguage: true,
    ...
    var langLinks = document.querySelectorAll(".header-lang__link");
    var savedLang = document.querySelector("[data-lang=".concat(lang, "]"));
    var getActiveLang = function getActiveLang() {
      return document.querySelector('.header-lang__link._active');
    };
    if (savedLang) {
      getActiveLang().classList.remove('_active');
      savedLang.classList.add('_active');
    }
    langLinks.forEach(function (el) {
      el.addEventListener("click", function (e) {
        e.preventDefault();
        var parent = e.target.closest('.header-lang__link');
        if (parent.classList.contains('_active')) return;
        var langContent = e.target.textContent.toLowerCase();
        getActiveLang().classList.remove('_active');
        parent.classList.add('_active');
        translator.load(langContent);
      });
    });
  }, 0);
  var showLink = document.querySelector('.-js-load-more');
  if (showLink) {
    var dons = toConsumableArray_default()(document.querySelectorAll('.info-list[class*="d-none"]'));
    var showDons = function showDons(elems) {
      return elems.forEach(function (el) {
        return el.classList.remove('d-none');
      });
    };
    showLink.addEventListener('click', function (e) {
      e.preventDefault();
      var showings = dons.splice(0, 4);
      showDons(showings);
      if (!dons.length) showLink.classList.add('d-none');
    });
  }
  ...
  // tooltips windows script
  var tooltips = document.querySelectorAll('.tip-info');
  var tipServer = document.querySelectorAll('.tip-server');
  var removeShow = function removeShow() {
    tipServer.forEach(function (el) {
      return el.classList.remove('_show');
    });
  };
  var getTip = function getTip(e) {
    return e.target.closest('.-js-tip');
  };
  function showToolTip(e) {
    e.preventDefault();
    var tip = getTip(e);
    var currentTip = toConsumableArray_default()(tipServer).indexOf(tip);
    var prevTip = toConsumableArray_default()(tipServer).filter(function (el, ind) {
      return ind !== currentTip;
    });
    prevTip[0].classList.remove('_show');
    if (tip.classList.contains('_show')) {
      tip.classList.remove('_show');
    } else {
      tip.classList.add('_show');
    }
    //console.log(tip.classList.contains('_show'))
  }
  ...
  // if (isMobile.any()) {
  // 	tipServer.forEach(el => {
  // 		el.onclick = e => showToolTip(e)
  // 	});
  // 	document.addEventListener(
  // 		"click",
  // 		function(event) {
  // 			if (!event.target.closest(".-js-tip")) {
  // 				removeShow()
  // 			}
  // 		},
  // 		false
  ...
  // else {
  // 	tooltips.forEach(el => {
  // 		el.onmouseover = e => showToolTip(e)
  // 		el.onmouseout = e => hideToolTip(e)
  // 	});
  // }

  // tabs-contents switches
  var tabsContainers = document.querySelectorAll('.-js-tabs');
  if (tabsContainers) {
    tabsContainers.forEach(function (el) {
      return tabSwitch(el);
    });
  }
  function tabSwitch(el) {
    var tabs = el.querySelectorAll('.-js-tab-link');
    var contents = el.querySelectorAll('.-js-tab-contents');
    tabs.forEach(function (el) {
      return el.addEventListener('click', function (e) {
        return showTab(e, tabs, contents);
      });
    });
  }
  var findActive = function findActive(elems, current) {
    var prevTab = toConsumableArray_default()(elems).find(function (el) {
      return el.classList.contains('_active');
    });
    var indexEl = toConsumableArray_default()(elems).indexOf(current);
    var indexElPrev = toConsumableArray_default()(elems).indexOf(prevTab);
    return {
      prevTab: prevTab,
      indexEl: indexEl,
      indexElPrev: indexElPrev
    };
  };
  function showTab(e, tabs, contents) {
    e.preventDefault();
    var tab = e.target;
    if (tab.classList.contains('_active')) return;
    var _findActive = findActive(tabs, tab),
      prevTab = _findActive.prevTab,
      indexEl = _findActive.indexEl,
      indexElPrev = _findActive.indexElPrev;
    prevTab.classList.remove('_active');
    tab.classList.add('_active');
    contents[indexElPrev].classList.remove('_active');
    contents[indexEl].classList.add('_active');
    setTimeout(function () {
      contents[indexElPrev].classList.remove('_visible');
      contents[indexEl].classList.add('_visible');
    }, 0);
  }
  var epicHall = document.querySelector('.epic-hall');
  var resizeHall = function resizeHall(epicHall) {
    //const dimensions = epicHall.getBoundingClientRect();
    var colInfo = epicHall.querySelectorAll('.col-info');
    var colInfoH = colInfo[0].getBoundingClientRect().height;
    var epicWinners = document.querySelector('.epic-winners');
    var infoList = epicWinners.querySelectorAll('.info-list');
    var infoListH = infoList[0].getBoundingClientRect().height;
    var max = Math.max(colInfoH, infoListH);
    if (max) {
      colInfo[0].style.height = max + 'px';
      infoList[0].style.height = max + 'px';
      infoList[1].style.height = max + 'px';
    }
    //const height = dimensions.height;
    //epicWinners.style.height = height + 'px';
  };
  ...
  var server1 = 'https://profile.elmorelab.com//bartz_x3/statistics/getOnlineNow?serverId=1';
  var server2 = 'https://profile.elmorelab.com//teon_x1/statistics/getOnlineNow?serverId=1';
  var serverTeon = document.querySelector('.-server-teon');
  var serverBartz = document.querySelector('.-server-bartz');
  function fetchServerOnlineJSON(_x) {
    return _fetchServerOnlineJSON.apply(this, arguments);
  }
  function _fetchServerOnlineJSON() {
    _fetchServerOnlineJSON = asyncToGenerator_default()( /*#__PURE__*/regenerator_default.a.mark(function _callee(path) {
      var response, online;
      return regenerator_default.a.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 5:
              ...
              return _context.abrupt("return", online);
            case 7:
            case "end":
              return _context.stop();
          }
  ...
  function getServerElems(server) {
    var serverOnlineTotal = server.querySelectorAll('.-online-total');
    var serverOnline = server.querySelector('.-online-players');
    var serverTraders = server.querySelector('.-online-traders');
    return {
      serverOnlineTotal: serverOnlineTotal,
      serverOnline: serverOnline,
      serverTraders: serverTraders
    };
  }
  fetchServerOnlineJSON(server1, serverBartz).then(function (json) {
    if (json.total) {
      var _getServerElems = getServerElems(serverBartz),
        serverOnlineTotal = _getServerElems.serverOnlineTotal,
        serverOnline = _getServerElems.serverOnline,
        serverTraders = _getServerElems.serverTraders;
      serverOnlineTotal.forEach(function (el) {
        return el.textContent = json.total;
      });
      serverOnline.textContent = json.onlineUsers;
      serverTraders.textContent = json.offlineTraders;
    }
  });
  fetchServerOnlineJSON(server2, serverTeon).then(function (json) {
    if (json.total) {
      var _getServerElems2 = getServerElems(serverTeon),
        serverOnlineTotal = _getServerElems2.serverOnlineTotal,
        serverOnline = _getServerElems2.serverOnline,
        serverTraders = _getServerElems2.serverTraders;
      serverOnlineTotal.forEach(function (el) {
        return el.textContent = json.total;
      });
      serverOnline.textContent = json.onlineUsers;
  ...
  var donationProgress1 = "https://profile.elmorelab.com/teon_x1/statistics/getDonationProgress";
  var donationProgress2 = "https://profile.elmorelab.com/bartz_x3/statistics/getDonationProgress";
  var donationProgressTotal = document.querySelector('.-donation-progress');
  var donationProgressLength = document.querySelector('.-donation-progress-fill');
  var donationBartz;
  var donationTeon;

  function fetchServerProgress(_x) {
    return _fetchServerProgress.apply(this, arguments);
  }
  function _fetchServerProgress() {
    _fetchServerProgress = asyncToGenerator_default()( /*#__PURE__*/regenerator_default.a.mark(function _callee(path) {
  ...
  fetchServerProgress(donationProgress1, donationBartz).then(function (json) {
      donationBartz = json.progress ? json.progress : 0;
      fetchServerProgress(donationProgress2, donationTeon).then(function (json2) {
        donationTeon = json2.progress ? json2.progress : 0;
        var total = (donationBartz + donationTeon).toFixed(2);
        donationProgressTotal.innerHTML = total + '%';
        donationProgressLength.style.setProperty('width', 'calc(' + total + '% + 1px)');
    });
  });
